...
{
	public class RenPyViewEnemy : MonoBehaviour
	{
		...
		void Update()
		{
			...
			case RenPyStatementType.SAY:
				...
				if(m_display.m_voice.m_source.isPlaying){
					float wait_time = m_display.m_voice.m_source.clip.length;
					float elapsed_time = m_display.m_voice.m_source.time;
			...
			case RenPyStatementType.LABEL:
				var lName = m_display.GetCurrentStatement() as RenPyLabel;
				//Debug.Log ("Label: " + lName );
				if(lName.Equals ("end")){
					//Debug.Log ("turnonInteract set to true");
					turnonInteract = true;
				}
				m_display.NextStatement();
				break;
